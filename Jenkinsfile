pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
          ...
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-pwd', passwordVariable: 'Pehu@12345', usernameVariable: 'richasahai')]) {
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe221 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe2211 -p 8083:80 richasahai/2febimg:v1'
                    sshagent(['sshkeypair']) {
